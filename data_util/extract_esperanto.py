    ...
    def run_step(self):
        ...
        if self.terminated:
            ...
            if self.opt.asr_save_feats:
                ...
                # save to a npy file
                if 'esperanto' in self.opt.asr_model:
                    if self.opt.out_name == '':
                        output_path = self.opt.asr_wav.replace('.wav', '_esperanto.npy')
                    else:
                        output_path = self.opt.out_name
                else:
                    output_path = self.opt.asr_wav.replace('.wav', '.npy')
                np.save(output_path, unfold_feats.cpu().numpy()) # [T, W=16, C=44]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wav', type=str, default='data/raw/val_wavs/intro.wav')
