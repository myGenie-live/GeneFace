...
from modules.radnerfs.utils import get_audio_features
...
class NeRFGUI:
    def __init__(self, hparams, task, dataset, debug=True):
        self.hparams_bak = hparams # shared with the trainer's opt to support in-place modification of rendering parameters.
        self.hparams = copy.deepcopy(hparams) # shared with the trainer's opt to support in-place modification of rendering parameters.
        self.W = self.hparams['gui_w']
        self.H = self.hparams['gui_h']
        self.cam = OrbitCamera(self.W, self.H, r=self.hparams['gui_radius'], fovy=self.hparams['gui_fovy'])
        self.debug = debug
        self.training = False
        ...
        self.W = dataset.W
        self.H = dataset.H
        self.cam.update_intrinsics(dataset.intrinsics)
        # use dataloader's pose
        pose_init = dataset.poses[0]
        self.cam.update_pose(pose_init)
        self.cam_t0 = copy.deepcopy(self.cam)
        ...
        self.cond_features = dataset.conds # [N, 29, 16]
        self.cond_idx = 0
        ...
        self.ind_index = 0
        self.ind_num = self.task.model.individual_embedding_num
    ...
    def register_dpg(self):
            ...
            with dpg.collapsing_header(label="Options", default_open=True):
                ...
                with dpg.group(horizontal=True):
                    ...
                    def callback_reset_pose(sender, app_data):
                        self.cam = copy.deepcopy(self.cam_t0)
                ...
                def callback_set_audio_index(sender, app_data):
                    self.cond_idx = app_data
                    self.need_update = True

                dpg.add_slider_int(label="Audio", min_value=0, max_value=self.cond_features.shape[0] - 1, format="%d", default_value=self.cond_idx, callback=callback_set_audio_index)

                # ind code index slider
                if hparams['individual_embedding_num'] > 0:
                ...
                def callback_set_dt_gamma(sender, app_data):
                    self.hparams['dt_gamma'] = app_data
                    self.need_update = True

                dpg.add_slider_float(label="dt_gamma", min_value=0, max_value=0.1, format="%.5f", default_value=self.hparams['dt_gamma'], callback=callback_set_dt_gamma)

                # max_steps slider
                def callback_set_max_steps(sender, app_data):
                    self.hparams['max_steps'] = app_data
                    self.need_update = True

                dpg.add_slider_int(label="max steps", min_value=1, max_value=1024, format="%d", default_value=self.hparams['max_steps'], callback=callback_set_max_steps)

                # aabb slider
                def callback_set_aabb(sender, app_data, user_data):
                    # user_data is the dimension for aabb (xmin, ymin, zmin, xmax, ymax, zmax)
                    self.task.model.aabb_infer[user_data] = app_data
                ...
                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="x", width=150, min_value=-self.hparams['bound'], max_value=0, format="%.2f", default_value=-self.hparams['bound'], callback=callback_set_aabb, user_data=0)
                    dpg.add_slider_float(label="", width=150, min_value=0, max_value=self.hparams['bound'], format="%.2f", default_value=self.hparams['bound'], callback=callback_set_aabb, user_data=3)

                with dpg.group(horizontal=True):
                    dpg.add_slider_float(label="y", width=150, min_value=-self.hparams['bound'], max_value=0, format="%.2f", default_value=-self.hparams['bound'], callback=callback_set_aabb, user_data=1)
                    dpg.add_slider_float(label="", width=150, min_value=0, max_value=self.hparams['bound'], format="%.2f", default_value=self.hparams['bound'], callback=callback_set_aabb, user_data=4)
        ...
        dpg.create_viewport(title='GeneFace-S', width=1080, height=720, resizable=True)
...
if __name__ == '__main__':
    ...
    from inference.nerfs.lm3d_radnerf_infer import LM3d_RADNeRFInfer
    set_hparams()

    inferencer = LM3d_RADNeRFInfer(hparams)
    nerf_task = inferencer.build_nerf_task().cuda()
    dataset = inferencer.dataset
    if hparams['infer_cond_name'] != '':
        cond = np.load(hparams['infer_cond_name'])[0] # [T, w=16, c=29]
        assert hparams['cond_type'] == 'idexp_lm3d_normalized'
        idexp_lm3d = torch.from_numpy(cond).float() 
        idexp_lm3d_mean = dataset.idexp_lm3d_mean
        idexp_lm3d_std = dataset.idexp_lm3d_std
        idexp_lm3d_normalized = (idexp_lm3d.reshape([-1,68,3]) - idexp_lm3d_mean)/idexp_lm3d_std 
        cond_win = idexp_lm3d_normalized.reshape([-1, 1, 204]) # [T, 1, 204]
        dataset.conds = cond_win
        dataset.samples = dataset.samples[:len(cond_win)]

    if hparams['amp']:
        nerf_task = nerf_task.half()
    with NeRFGUI(hparams, nerf_task, dataset) as gui:
        gui.render()
    print("GoodBye!")